...
import type { GetHowToBuyResponse } from "@/services/how-to-buy/types";

type Locale = "id" | "en";

const clampLocale = (value?: string): Locale => (value === "en" ? "en" : "id");

const HowToBuyClient = () => {
  const t = useTranslations("HowToBuy");
  const params = useParams<{ locale: string }>();
  const locale = clampLocale(params?.locale);

  const howToBuyQuery = useApiQuery<GetHowToBuyResponse>({
    key: ["how-to-buy", locale],
    endpoint: "/web/cara-pembelian",
    searchParams: { locale },
  });

  const data = howToBuyQuery.data?.data ?? null;
