...
import type { GetPrivacyPolicyResponse } from "@/services/privacy-policy/types";
...
const PrivacyPolicyClient = () => {
  ...
  const privacyQuery = useApiQuery<GetPrivacyPolicyResponse>({
    key: ["privacy-policy", locale],
    endpoint: "/web/kebijakan-privasi",
    searchParams: { locale },
  });

  const data = privacyQuery.data?.data ?? null;
        ...
        <div className="mx-auto w-full max-w-[840px]">
          ...
          <div className="mt-6 flex items-center gap-3">
            <div className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-[#ffcf02]">
              <ShieldCheck className="size-5 text-black" strokeWidth={2} />
            </div>
            <h1 className="text-2xl font-bold text-black md:text-3xl">
              {privacyQuery.isLoading ? t("heading") : (data?.judul ?? t("heading"))}
            </h1>
          </div>
          ...
          <div className="mt-10 rounded-2xl border border-gray-200 bg-white p-6 shadow-[0_4px_8px_rgba(0,0,0,0.05)] md:p-10">
            ...
            ) : privacyQuery.isError || !data?.konten ? (
